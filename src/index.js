...
form.addEventListener("submit", renderImages);
 ...
 function renderImages(ev){
  ...
  getImages(ev).then( response => { 
    ...
    }else {Notify.success(`Hooray! We found ${response.data.totalHits} images`);}
  response.data.hits.forEach(item => {
    console.log(item);
     backup += `<div class="photo-card">
    <a class="gallery__item" href=${item.largeImageURL}><img class="gallery__image" src=${item.previewURL
  ...
  function onLoadMore(ev) {
    ...
    response.data.hits.forEach(item => {
             backup += `<div class="photo-card">
      <a class="gallery__item" href=${item.largeImageURL}><img class="gallery__image" src=${item.previewURL
      } alt=${item.description} loading="lazy"/></a>
    <div class="info">
      <p class="info-item">
        <b>Likes: ${item.likes}</b>
      </p>
